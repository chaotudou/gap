...
outfile = open ("H:\GAP_prediction_cum.txt", "w")
...
header = "\nWell\tOil Cum(MMbbl)\tGas Cum(MMscf)\n\n"
...
# Get cumulative oil
        ...
        qo_rate_string = 'GAP.MOD[{PROD}].WELL[{%s}].PREDRES[{01/01/2030}].CUMOIL' % (well)
        qo_rate = GAP.GetValue(qo_rate_string)

# Get cumulative gas

        gl_rate_string = 'GAP.MOD[{PROD}].WELL[{%s}].PREDRES[{01/01/2030}].CUMGAS' % (well)
