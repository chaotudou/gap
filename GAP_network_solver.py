...
outfile = open ("H:/GAP_solver.txt", "w")
...
header = "\nWell\tOil Rate(bbl/day)\tGas Rate(MMscf/day)\n\n"
...
# Get oil rate
        ...
        qo_rate_string = 'GAP.MOD[{PROD}].WELL[{%s}].SolverResults[0].QoilAvg' % (well)
        qo_rate = GAP.GetValue(qo_rate_string)

# Get gas rate

        gl_rate_string = 'GAP.MOD[{PROD}].WELL[{%s}].SolverResults[0].QgasAvg' % (well)
